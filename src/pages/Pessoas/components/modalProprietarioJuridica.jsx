  ...
  useEffect(() => {
    if (selectedImovel) {
      const proprietariosDoImovel = selectedImovel.imoveisProprietarios.map(
        (proprietario) => ({
          nome:
            proprietario.pessoa && proprietario.pessoa.nome
              ? proprietario.pessoa.nome
              : proprietario.pessoaJuridica.nomeFantasia || proprietario.pessoaJuridica.razaoSocial, // Adicione a razão social aqui
          percentual: proprietario.percentualPropriedade,
        })
      );
      console.log(proprietariosDoImovel);
